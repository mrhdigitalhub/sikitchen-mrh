...
export default function DashboardMain() {
  ...
  useEffect(() => {
    ...
    setProfile(sess)
    // Load company settings dari localStorage + Supabase
    const saved = localStorage.getItem('sikitchen_company')
    if (saved) {
      try { setCompany(JSON.parse(saved)) } catch {}
    }
    // Load dari Supabase perusahaan_settings jika ada
...
function DashboardAdmin({ profile, company, setCompany }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* NEW: Company Settings - Logo & Nama Perusahaan */}
...
function CompanySettingsManager({ company, setCompany, profile }) {
  ...
  async function handleLogoChange(e) {
    ...
    if (!file) return
    setUploading(true)
    // Convert to base64 for quick preview + localStorage (cepat, tanpa storage bucket)
    const reader = new FileReader()
    reader.onload = async () => {
      const base64 = reader.result
      setLogoPreview(base64)
      const newComp = { nama: nama, logo: base64 }
      setCompany(newComp)
      localStorage.setItem('sikitchen_company', JSON.stringify(newComp))
      // Simpan ke Supabase jika ada table perusahaan_settings
      try {
        await supabase.from('perusahaan_settings').upsert({ 
          perusahaan: profile?.perusahaan || nama, 
          nama_perusahaan: nama,
          logo_url: base64,
          updated_at: new Date().toISOString()
        }, { onConflict: 'perusahaan' })
      } catch(err) { console.log('Simpan lokal saja', err) }
      setUploading(false)
    }
  ...
  async function saveNama() {
    ...
    const newComp = { nama: nama.trim(), logo: logoPreview }
    setCompany(newComp)
    localStorage.setItem('sikitchen_company', JSON.stringify(newComp))
    // Update profiles perusahaan field juga
    try {
      await supabase.from('perusahaan_settings').upsert({ 
        perusahaan: profile?.perusahaan || nama, 
      ...
      alert(`Nama perusahaan disimpan: ${nama.trim()} - Owner akan lihat Performance Catering - ${nama.trim()}`)
    } catch(e) {
      alert(`Nama perusahaan disimpan lokal: ${nama.trim()}`)
    }
  }

  return (
    <div className="bg-white p-4 rounded-xl border-2 border-[#D4AF37]/30 shadow-sm">
      <h3 className="font-semibold mb-1 flex items-center gap-2">🏢 Pengaturan Perusahaan - Header Owner</h3>
      <p className="text-[11px] text-slate-500 mb-4">Atur nama & logo perusahaan - otomatis tampil di header Owner sebagai <b>Performance Catering - (Nama Perusahaan)</b></p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="col-span-1">
          <label className="text-xs font-semibold">Logo Perusahaan</label>
          <div className="mt-2 border-2 border-dashed rounded-xl p-4 text-center bg-slate-50">
            {logoPreview ? <img src={logoPreview} alt="Logo" className="w-24 h-24 object-contain mx-auto rounded-lg mb-2" /> : <div className="w-24 h-24 bg-slate-200 rounded-lg mx-auto mb-2 flex items-center justify-center text-2xl">🏢</div>}
            <input type="file" accept="image/*" onChange={handleLogoChange} className="text-[11px] w-full" />
            <p className="text-[10px] text-slate-400 mt-1">{uploading ? 'Uploading...' : 'PNG/JPG max 2MB'}</p>
          </div>
        </div>
        <div className="col-span-2">
          <label className="text-xs font-semibold">Nama Perusahaan</label>
          <input value={nama} onChange={e=>setNama(e.target.value)} placeholder="Contoh: SIKITCHEN-MRH, Dapur Mama, Catering Berkah" className="w-full border p-3 rounded-xl mt-2 text-sm" />
          <div className="mt-3 bg-[#0A1931] text-white p-3 rounded-lg">
            <div className="text-[11px] text-white/60">Preview Header Owner:</div>
            <div className="text-sm font-bold mt-1 flex items-center gap-2">
              {logoPreview && <img src={logoPreview} className="w-6 h-6 object-contain rounded" />}
              Performance Catering - {nama || 'Nama Perusahaan'}
            </div>
            <div className="text-[11px] text-white/60 mt-1">Sel, 15 Sep 2026 | {nama} | OWNER</div>
          </div>
          <button onClick={saveNama} className="mt-3 w-full bg-[#D4AF37] text-[#0A1931] font-bold py-2.5 rounded-xl text-sm hover:bg-yellow-400">💾 Simpan Nama & Logo Perusahaan</button>
          <p className="text-[10px] text-slate-400 mt-2">Disimpan di localStorage + Supabase (jika ada table perusahaan_settings). Owner auto update.</p>
        </div>
      </div>
    </div>
  )
...
// ================= OWNER - PERFORMANCE CATERING HEADER =================
function DashboardOwnerBlueprint({ profile, company }) {
  ...
  useEffect(() => {
    ...
    fetchOwnerData()
    const interval = setInterval(fetchOwnerData, 30000)
  ...
  return (
    <div className="space-y-4 bg-[#F8FAFC] -m-6 p-6">
      {/* HEADER - GANTI JADI PERFORMANCE CATERING - NAMA PERUSAHAAN */}
      <div className="bg-[#0A1931] text-white rounded-xl p-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          {logoPerusahaan ? <img src={logoPerusahaan} alt="Logo" className="w-10 h-10 rounded-lg object-contain bg-white p-1" /> : <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center text-lg">🏢</div>}
          <div>
            <h1 className="text-lg font-bold">Performance Catering - {namaPerusahaan}</h1>
            <p className="text-xs text-white/60">Ringkasan Operasional & Keuangan Hari Ini | {now.toLocaleDateString('id-ID', { weekday:'short', day:'numeric', month:'short', year:'numeric' })} | SIKITCHEN-MRH | OWNER</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
