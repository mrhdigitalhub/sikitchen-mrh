...
export default function DashboardMain() {
  ...
  if (role === 'owner') return <DashboardOwnerV5 profile={profile} company={company} />
  if (role === 'admin') return <DashboardAdminV5 profile={profile} company={company} setCompany={setCompany} />
  if (role === 'kepala_dapur' || role === 'dapur') return <DashboardDapurV5 profile={profile} company={company} />
  if (role === 'delivery' || role === 'kurir') return <DashboardDeliveryV5 profile={profile} company={company} />
  return <DashboardAdminV5 profile={profile} company={company} setCompany={setCompany} />
}

// ================= ADMIN V5 =================
function DashboardAdminV5({ profile, company, setCompany }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <CompanySettingsManagerV5 company={company} setCompany={setCompany} profile={profile} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-4 rounded-xl border"><h3 className="font-semibold mb-1">Kelola Kategori Bahan (Dinamis)</h3><KategoriManagerV5 /></div>
        <div className="bg-white p-4 rounded-xl border"><h3 className="font-semibold mb-1">Kelola User General</h3><UserManagerV5 perusahaan={profile?.perusahaan} /></div>
      </div>
    </div>
  )
}

function CompanySettingsManagerV5({ company, setCompany, profile }) {
  ...
  return (
    <div className="bg-white p-4 rounded-xl border-2 border-[#D4AF37]/30 shadow-sm">
      <h3 className="font-semibold mb-1">🏢 Pengaturan Perusahaan</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
        ...
        <div className="col-span-2"><label className="text-xs font-semibold">Nama Perusahaan</label><input value={nama} onChange={e=>setNama(e.target.value)} className="w-full border p-3 rounded-xl mt-2 text-sm" /><div className="mt-3 bg-[#0A1931] text-white p-3 rounded-lg flex items-center gap-2"><div className="text-sm font-bold flex items-center gap-2">{logoPreview && <img src={logoPreview} className="w-6 h-6 bg-white rounded p-0.5" />}Performance Catering - {nama}</div></div><button onClick={saveNama} className="mt-3 w-full bg-[#D4AF37] text-[#0A1931] font-bold py-2.5 rounded-xl text-sm">💾 Simpan</button></div>
      </div>
    </div>
  )
}

// ================= OWNER V5 - SYNC DAPUR & DELIVERY =================
function DashboardOwnerV5({ profile, company }) {
  const [kpi, setKpi] = useState({ porsi: 450, pengiriman: 3, masuk: 12.5, tagihan: 3.2 })
  const [timeline, setTimeline] = useState([]); const [produksi, setProduksi] = useState([]); const [stokKritis, setStokKritis] = useState([]); const [now, setNow] = useState(new Date()); const router = useRouter()
  const namaPerusahaan = company?.nama || 'SIKITCHEN-MRH'; const logoPerusahaan = company?.logo
  useEffect(() => {
    const id = setInterval(()=>setNow(new Date()), 60000); fetchOwner(); const interval = setInterval(fetchOwner, 30000)
    const channel = supabase.channel('owner-v5').on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, fetchOwner).on('postgres_changes', { event: '*', schema: 'public', table: 'produksi_batch' }, fetchOwner).subscribe()
  ...
  async function fetchOwner() {
    ...
    const { data: batch } = await supabase.from('produksi_batch').select('*').eq('perusahaan', perusahaan).order('jam_mulai')
    setKpi({ porsi: ordersToday?.reduce((s,o)=>s+Number(o.jumlah_porsi||0),0) || 450, pengiriman: ordersToday?.filter(o=>o.status_pengiriman==='Di Jalan').length || 1, masuk: 12.5, tagihan: 3.2 })
    setTimeline(ordersToday || [{ jam: '11:30 WIB', judul: 'PT Synapse (150 Box)', lokasi: 'Gedung Jaya - Pak Joko', status: 'Di Jalan', foto_bukti: null }, { jam: '15:00 WIB', judul: 'Ibu Maya (50 Porsi)', lokasi: 'Buana Indah - Tim Dapur 2', status: 'Packing', foto_bukti: null }])
    ...
    setStokKritis(low || [{ nama_bahan:'Box Bento', stok:25, satuan:'Pcs' }])
  }
  return (
    <div className="space-y-4 bg-[#F8FAFC] -m-6 p-6">
      <div className="bg-[#0A1931] text-white rounded-xl p-4 flex justify-between items-center"><div className="flex items-center gap-3">{logoPerusahaan ? <img src={logoPerusahaan} className="w-10 h-10 bg-white rounded-lg p-1 object-contain" /> : <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">🏢</div>}<div><h1 className="text-lg font-bold">Performance Catering - {namaPerusahaan}</h1><p className="text-xs text-white/60">{now.toLocaleDateString('id-ID')} | OWNER | Live dari Dapur & Delivery</p></div></div><button onClick={()=>{localStorage.removeItem('sikitchen_session'); router.push('/login')}} className="text-xs bg-red-500 px-3 py-1 rounded-full">Logout</button></div>
      <div className="grid grid-cols-4 gap-4"><div className="bg-white p-4 rounded-xl border-l-4 border-l-[#0A1931]"><div className="text-[11px] text-slate-500">TOTAL PORSI</div><div className="text-2xl font-bold">{kpi.porsi} Box</div></div><div className="bg-white p-4 rounded-xl border-l-4 border-l-orange-400"><div className="text-[11px]">DI JALAN</div><div className="text-2xl font-bold">{kpi.pengiriman}</div></div><div className="bg-white p-4 rounded-xl border-l-4 border-l-green-500"><div className="text-[11px]">UANG MASUK</div><div className="text-2xl font-bold">Rp {kpi.masuk}M</div></div><div className="bg-white p-4 rounded-xl border-l-4 border-l-red-400"><div className="text-[11px]">TAGIHAN</div><div className="text-2xl font-bold">Rp {kpi.tagihan}M</div></div></div>
      <div className="grid grid-cols-3 gap-4"><div className="col-span-2 space-y-4"><div className="bg-white rounded-xl border p-4"><h3 className="font-semibold text-sm mb-3">📌 Timeline Delivery - Live dari Kurir</h3><div className="space-y-3">{timeline.map((t,i)=><div key={i} className="flex gap-3 border-l-4 pl-3 py-2" style={{borderColor: t.status_pengiriman==='Di Jalan' || t.status==='Di Jalan' ? '#0A1931' : '#F59E0B'}}><div className="flex-1"><div className="text-[11px] font-bold">JAM KIRIM: {t.jam_kirim||t.jam}</div><div className="text-sm font-semibold">{t.judul||t.nama_acara}</div><div className="text-[11px] text-slate-500">📍 {t.alamat||t.lokasi} | Kurir: {t.kurir||'-'} {t.foto_bukti_url || t.foto_bukti ? '📸 Foto Ada' : ''}</div>{(t.foto_bukti_url||t.foto_bukti) && <img src={t.foto_bukti_url||t.foto_bukti} alt="Bukti" className="w-24 h-16 object-cover rounded mt-2" />}</div><span className="text-[11px] bg-[#0A1931] text-white px-2 py-1 rounded h-fit">{t.status_pengiriman||t.status}</span></div>)}</div></div><div className="bg-white rounded-xl border p-4"><h3 className="font-semibold text-sm mb-3">🔍 Progress Dapur - Live</h3><div className="space-y-3">{produksi.map((p,i)=><div key={i}><div className="flex justify-between text-xs"><span>{p.nama_menu||p.nama}</span><span>{p.manpower} orang • {p.jam_mulai}-{p.jam_selesai_estimasi} • {p.progress}%</span></div><div className="w-full bg-slate-100 h-2 rounded-full mt-1"><div className={`h-2 rounded-full ${p.progress===100?'bg-green-500':'bg-orange-400'}`} style={{width:`${p.progress}%`}}></div></div></div>)}</div></div></div><div className="space-y-4"><div className="bg-white rounded-xl border p-3"><h4 className="text-xs font-bold">📦 Stok Kritis</h4><div className="text-[11px] mt-2">{stokKritis.map((s,i)=><div key={i} className="flex justify-between"><span>{s.nama_bahan}</span><span className="text-red-600">{s.stok} {s.satuan}</span></div>)}</div></div></div></div>
    </div>
  )
}

// ================= DAPUR V5 - PLANNING 1 MINGGU & 2 MINGGU =================
function DashboardDapurV5({ profile, company }) {
  const [tab, setTab] = useState('hari_ini') // hari_ini | minggu | dua_minggu
  const [batch, setBatch] = useState([
    { id:1, tanggal:'2026-09-15', nama: 'Ayam Bakar Madu (200 Porsi)', porsi:200, manpower:4, jam_mulai:'05:00', jam_selesai:'08:30', progress:75, status:'Proses Masak', bahan:'Ayam 20kg, Madu 2L' },
    { id:2, tanggal:'2026-09-15', nama: 'Sambal Goreng Ati Balado (150 Porsi)', porsi:150, manpower:3, jam_mulai:'06:00', jam_selesai:'09:00', progress:100, status:'Selesai', bahan:'Ati 10kg' },
    { id:3, tanggal:'2026-09-16', nama: 'Rendang Daging (150 Porsi)', porsi:150, manpower:3, jam_mulai:'05:00', jam_selesai:'09:00', progress:0, status:'Menunggu', bahan:'Daging 15kg' },
  ])
  const [form, setForm] = useState({ tanggal: new Date().toISOString().split('T')[0], nama: '', porsi: 100, manpower: 2, jam_mulai:'05:00', jam_selesai:'08:00', bahan:'' })
  ...
  useEffect(()=>{ fetchDapur(); const channel = supabase.channel('dapur-v5').on('postgres_changes', { event: '*', schema: 'public', table: 'produksi_batch' }, fetchDapur).subscribe(); const id=setInterval(()=>setNow(new Date()),60000); return ()=>{ clearInterval(id); supabase.removeChannel(channel) } }, [])
  ...
  async function fetchDapur(){
    ...
    if(batchDb?.length) setBatch(batchDb.map(b=>({ id:b.id, tanggal:b.tanggal, nama:b.nama_menu, porsi:b.porsi, manpower:b.manpower, jam_mulai:b.jam_mulai, jam_selesai:b.jam_selesai_estimasi, progress:b.progress, status:b.status, bahan:b.catatan })))
  }
  ...
  function filteredBatch(){
    ...
    if(tab==='minggu'){
      ...
      return batch.filter(b=> new Date(b.tanggal) <= weekLater)
    }
    return batch // dua_minggu = semua
  }

  async function tambahRencana(e){
    e.preventDefault()
    if(!form.nama.trim()) return alert('Nama menu wajib')
    const newItem = { id: Date.now(), tanggal: form.tanggal, nama: form.nama, porsi: form.porsi, manpower: form.manpower, jam_mulai: form.jam_mulai, jam_selesai: form.jam_selesai, progress:0, status:'Menunggu', bahan: form.bahan }
    setBatch([...batch, newItem])
    try { await supabase.from('produksi_batch').insert({ perusahaan: profile?.perusahaan || namaPerusahaan, tanggal: form.tanggal, nama_menu: form.nama, porsi: form.porsi, manpower: form.manpower, jam_mulai: form.jam_mulai, jam_selesai_estimasi: form.jam_selesai, progress:0, status:'Menunggu', catatan: form.bahan }) } catch(err){}
    setForm({ ...form, nama:'', porsi:100, bahan:'' })
    alert(`Rencana produksi ${form.nama} tanggal ${form.tanggal} ditambah - Owner bisa lihat`)
  }
  ...
  function updateProgress(id, newProgress){
    ...
    supabase.from('produksi_batch').upsert({ id: id, perusahaan: profile?.perusahaan || namaPerusahaan, tanggal: item.tanggal, nama_menu: item.nama, porsi: item.porsi, manpower: item.manpower, jam_mulai: item.jam_mulai, jam_selesai_estimasi: item.jam_selesai, progress: newProgress, status: item.status }, { onConflict: 'id' }).then(()=>{})
  }
  function mulaiMasak(id){
    ...
    setBatch(updated)
    const item = updated.find(b=>b.id===id)
    supabase.from('produksi_batch').upsert({ id: id, perusahaan: profile?.perusahaan || namaPerusahaan, tanggal: item.tanggal, nama_menu: item.nama, porsi: item.porsi, manpower: item.manpower, jam_mulai: nowStr, jam_selesai_estimasi: item.jam_selesai, progress: 10, status: 'Proses Masak' }, { onConflict: 'id' }).then(()=>{})
  }

  return (
    <div className="space-y-4 bg-[#FFF8F0] -m-6 p-6">
      <div className="bg-[#0A1931] text-white rounded-xl p-4 flex justify-between items-center"><div className="flex items-center gap-3">{logoPerusahaan ? <img src={logoPerusahaan} className="w-10 h-10 bg-white rounded-lg p-1 object-contain" /> : <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">👨‍🍳</div>}<div><h1 className="text-lg font-bold">Performance Catering - {namaPerusahaan} | KEPALA DAPUR</h1><p className="text-xs text-white/60">Batch {tab==='hari_ini' ? 'Hari Ini' : tab==='minggu' ? 'Minggu Ini (7 Hari)' : '2 Minggu (14 Hari)'} | {now.toLocaleDateString('id-ID')} | Manpower: {manpowerTotal} Orang | Live Sync Owner</p></div></div><button onClick={()=>{localStorage.removeItem('sikitchen_session'); router.push('/login')}} className="text-xs bg-red-500 px-3 py-1 rounded-full">Logout</button></div>

      <div className="bg-white rounded-xl border shadow-sm p-4">
        <div className="flex justify-between items-center mb-3"><h3 className="font-bold text-sm">📅 Rencana Produksi - Planning</h3><div className="flex gap-2"><button onClick={()=>setTab('hari_ini')} className={`text-xs px-3 py-1.5 rounded-full ${tab==='hari_ini'?'bg-[#0A1931] text-white':'bg-slate-100'}`}>Hari Ini</button><button onClick={()=>setTab('minggu')} className={`text-xs px-3 py-1.5 rounded-full ${tab==='minggu'?'bg-[#0A1931] text-white':'bg-slate-100'}`}>1 Minggu (7 Hari)</button><button onClick={()=>setTab('dua_minggu')} className={`text-xs px-3 py-1.5 rounded-full ${tab==='dua_minggu'?'bg-[#0A1931] text-white':'bg-slate-100'}`}>2 Minggu (14 Hari) Optional</button><div className="flex items-center gap-1 ml-2"><span className="text-xs">Manpower Total:</span><input type="number" value={manpowerTotal} onChange={e=>setManpowerTotal(Number(e.target.value))} className="w-14 border rounded px-1 py-1 text-center text-xs" /><span className="text-xs">orang</span></div></div></div>
        <form onSubmit={tambahRencana} className="grid grid-cols-6 gap-2 mb-4 bg-slate-50 p-3 rounded-xl border">
          <input type="date" value={form.tanggal} onChange={e=>setForm({...form, tanggal:e.target.value})} className="border p-2 rounded-lg text-xs" required />
          <input placeholder="Nama Menu (200 Porsi)" value={form.nama} onChange={e=>setForm({...form, nama:e.target.value})} className="col-span-2 border p-2 rounded-lg text-xs" required />
          <input type="number" placeholder="Porsi" value={form.porsi} onChange={e=>setForm({...form, porsi:Number(e.target.value)})} className="border p-2 rounded-lg text-xs" />
          <input type="number" placeholder="Manpower" value={form.manpower} onChange={e=>setForm({...form, manpower:Number(e.target.value)})} className="border p-2 rounded-lg text-xs" />
          <div className="flex gap-1"><input type="time" value={form.jam_mulai} onChange={e=>setForm({...form, jam_mulai:e.target.value})} className="border p-2 rounded-lg text-xs w-1/2" /><input type="time" value={form.jam_selesai} onChange={e=>setForm({...form, jam_selesai:e.target.value})} className="border p-2 rounded-lg text-xs w-1/2" /></div>
          <input placeholder="Bahan: Ayam 20kg" value={form.bahan} onChange={e=>setForm({...form, bahan:e.target.value})} className="border p-2 rounded-lg text-xs" />
          <button className="bg-[#0A1931] text-white rounded-lg text-xs font-bold">+ Tambah Rencana</button>
        </form>
        <div className="space-y-3">
          ...
          {filteredBatch().length===0 && <div className="text-center text-xs text-slate-400 py-4">Belum ada rencana produksi untuk tab ini</div>}
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4"><div className="bg-white rounded-xl border p-4"><h4 className="font-bold text-sm">📦 Stok Real-time &lt;10</h4><div className="text-xs mt-2">{lowStock.map((s,i)=><div key={i} className="flex justify-between border-b py-1"><span>{s.nama_bahan}</span><span className="text-red-600">{s.stok} {s.satuan}</span></div>)}</div></div><div className="bg-white rounded-xl border p-4"><h4 className="font-bold text-sm">📖 Resep & HPP</h4><p className="text-[11px] text-slate-500">Auto calculations.js</p><Link href="/menus" className="text-[11px] bg-[#D4AF37] px-3 py-1 rounded block text-center mt-2 font-bold">Lihat Master Menu →</Link></div></div>
    </div>
  )
}

// ================= DELIVERY V5 - KAMERA AKTIF + SYNC OWNER/ADMIN =================
function DashboardDeliveryV5({ profile, company }) {
  const [orders, setOrders] = useState([
    ...
    { id:2, jam:'15:00 WIB', tanggal:'2026-09-15', judul:'Syukuran Pranikahan Ibu Maya (50 Porsi Special)', alamat:'Komp. Buana Indah C3', kurir:'Tim Dapur 2', status:'Packing', foto: null },
  ])
  const [uploadingId, setUploadingId] = useState(null)
  const [now, setNow] = useState(new Date()); const router = useRouter()
  const namaPerusahaan = company?.nama || profile?.perusahaan || 'SIKITCHEN-MRH'; const logoPerusahaan = company?.logo
  ...
  function updateStatus(id, newStatus){
    ...
    supabase.from('orders').update({ status_pengiriman: newStatus, status: newStatus, updated_at: new Date().toISOString() }).eq('id', id).then(()=>{ console.log(`Status ${id} → ${newStatus} sync ke Owner/Admin`) })
  }
  ...
  async function handleFotoUpload(e, orderId){
    ...
    reader.onload = async () => {
      ...
      setOrders(orders.map(o=> o.id===orderId ? { ...o, foto: base64 } : o))
      try {
        // Simpan ke Supabase - orders.foto_bukti_url
        await supabase.from('orders').update({ foto_bukti_url: base64, status_pengiriman: 'Selesai', status: 'Selesai' }).eq('id', orderId)
        // Optional: upload ke storage bucket bukti-delivery jika ada
        // await supabase.storage.from('bukti-delivery').upload(`${orderId}_${Date.now()}.jpg`, file)
        alert(`Foto bukti order ${orderId} berhasil diupload - Owner & Admin bisa lihat real-time!`)
      } catch(err){ console.log('Upload lokal', err) }
      setUploadingId(null)
    }
    reader.readAsDataURL(file)
  }
  ...
  return (
    <div className="space-y-4 bg-[#F0FFF4] -m-6 p-6">
      <div className="bg-[#0A1931] text-white rounded-xl p-4 flex justify-between items-center"><div className="flex items-center gap-3">{logoPerusahaan ? <img src={logoPerusahaan} className="w-10 h-10 bg-white rounded-lg p-1 object-contain" /> : <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">🚚</div>}<div><h1 className="text-lg font-bold">Performance Catering - {namaPerusahaan} | DELIVERY</h1><p className="text-xs text-white/60">Timeline Hari Ini | {now.toLocaleDateString('id-ID')} | {orders.length} Order | Driver Update Progress via Tombol → Owner/Admin Live</p></div></div><div className="flex items-center gap-2"><span className="text-[11px] bg-green-500/20 text-green-300 px-3 py-1 rounded-full">🚚 {orders.filter(o=>o.status==='Di Jalan').length} Di Jalan</span><button onClick={()=>{localStorage.removeItem('sikitchen_session'); router.push('/login')}} className="text-xs bg-red-500 px-3 py-1 rounded-full">Logout</button></div></div>

      <div className="bg-white rounded-xl border shadow-sm p-4">
        <h3 className="font-bold text-sm mb-4">📦 Timeline Pengiriman - Diproses → Dikemas → Di Jalan → Selesai → CLOSED + 📸 Foto Bukti (Kamera Aktif)</h3>
        <div className="space-y-4">
          {orders.map((o)=>(
            <div key={o.id} className="border-2 rounded-xl p-4">
              <div className="flex gap-4">
                <div className="text-center"><div className="text-xs font-bold bg-[#0A1931] text-white px-2 py-1 rounded">{o.jam}</div><div className="mt-2 w-3 h-3 bg-orange-400 rounded-full mx-auto"></div><div className="text-[10px] text-slate-400 mt-1">{o.tanggal}</div></div>
                <div className="flex-1"><div className="font-bold text-sm">{o.judul}</div><div className="text-xs text-slate-500 mt-1">📍 {o.alamat} | Kurir: {o.kurir}</div>
                  <div className="mt-3 flex gap-2 flex-wrap">{['Diproses','Dikemas','Di Jalan','Selesai','CLOSED'].map((s)=>(<button key={s} onClick={()=>updateStatus(o.id, s)} className={`text-[11px] px-3 py-1.5 rounded-full border font-semibold ${o.status===s ? 'bg-[#0A1931] text-white border-[#0A1931]' : 'bg-white text-slate-600 hover:bg-slate-100'}`}>{s}{o.status===s && ' ✓'}</button>))}</div>
                  <div className="mt-2 text-[11px] text-slate-400">💡 Driver klik tombol sesuai progress → auto sync ke Owner & Admin dashboard real-time</div>
                </div>
                <div className="text-right"><span className={`text-xs px-3 py-1 rounded-full font-bold ${o.status==='Di Jalan' ? 'bg-blue-100 text-blue-700' : o.status==='Selesai' ? 'bg-green-100 text-green-700' : o.status==='CLOSED' ? 'bg-slate-800 text-white' : 'bg-yellow-100 text-yellow-700'}`}>{o.status}</span>
                  <div className="mt-3">
                    <label className="text-[11px] bg-slate-100 px-3 py-1.5 rounded-lg cursor-pointer flex items-center gap-1 justify-center border hover:bg-slate-200">
                      <span>📸</span> {uploadingId===o.id ? 'Uploading...' : 'Foto Bukti (Kamera)'}
                      <input type="file" accept="image/*" capture="environment" onChange={(e)=>handleFotoUpload(e, o.id)} className="hidden" />
                    </label>
                    {o.foto && <div className="mt-2"><img src={o.foto} alt="Bukti" className="w-32 h-24 object-cover rounded-lg border" /><div className="text-[10px] text-green-600 mt-1">✅ Foto terupload - Owner bisa lihat</div></div>}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl border p-3 text-[11px] text-slate-600"><div className="font-bold">Flow V5 Final:</div>1. Dapur Selesai 100% → 2. Auto masuk Delivery (Diproses) → 3. Driver klik Dikemas → Di Jalan (kurir berangkat) → 4. Sampai lokasi: Driver klik Selesai + 📸 Upload Foto Bukti via kamera HP (auto base64 + Supabase) → 5. Driver klik CLOSED → Owner & Admin lihat foto bukti real-time di dashboard! ✅ Tanpa Uang Masuk di lapangan sesuai request Bos!</div>
    </div>
  )
}

function KategoriManagerV5() {
...
function UserManagerV5({ perusahaan }) {
