...
export default function DashboardAdmin(){
    ...
    async function load(){
      try{
        ...
        setStats({
          ...
          users: 5,
          orders: orders.data ? orders.data.filter(function(o){return o.status!=='CLOSED'}).length : 1,
          closed: orders.data ? orders.data.filter(function(o){return o.status==='CLOSED'}).length : 1
        })
      }catch(e){
        console.log(e)
      }
  ...
  function handleSimpan(){
    ...
    alert('Disimpan! Perusahaan: ' + cleanName + ' Alamat: ' + cleanAlamat)
  }
  ...
  return (
    <div className="p-4 md:p-7 space-y-6 bg-slate-50 min-h-screen" style={{fontFamily:'Inter, Arial, sans-serif', fontSize:'15px'}}>
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl border shadow-sm">
        <div><div className="font-black text-[24px]">Dashboard Admin</div><div className="text-[14px] font-bold text-slate-600">SIKITCHEN-MRH | ADMIN - Build Fix - 17/09/2026</div></div>
      ...
      <div className="bg-white rounded-3xl border-2 border-slate-200 p-6 shadow-sm">
        <div className="font-black text-[16px] mb-5">Pengaturan Perusahaan - Build Fix (No TS)</div>
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-[12px] mb-4">Build Error Fixed: Hapus `as string` TypeScript, pakai JS murni. Vercel harus ijo.</div>
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div><div className="text-[13px] font-black mb-1.5">Nama Perusahaan (Max 50)</div><input value={company} onChange={function(e){ setCompany(e.target.value.slice(0,50)) }} className="w-full border-2 rounded-xl px-4 py-3 text-[14px] font-bold focus:border-blue-400 outline-none" placeholder="SIKITCHEN-MRH" maxLength={50}/><div className="text-[11px] text-slate-500 mt-1">{company.length}/50 huruf</div></div>
            <div><div className="text-[13px] font-black mb-1.5">Alamat Perusahaan</div><textarea value={alamat} onChange={function(e){ setAlamat(e.target.value.slice(0,100)) }} className="w-full border-2 rounded-xl px-4 py-3 text-[14px] focus:border-blue-400 outline-none" rows={2} placeholder="Jl. Catering No.1 Bandung" maxLength={100}></textarea><div className="text-[11px] text-slate-500">{alamat.length}/100</div></div>
            <button onClick={function(){ setShowPerf(true) }} className="w-full bg-[#0A1931] text-white rounded-xl px-4 py-3.5 text-[14px] font-black hover:bg-black shadow-lg">Performance - {company.slice(0,30)}</button>
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={handleReset} className="bg-red-100 border-2 border-red-300 text-red-700 font-black py-3.5 rounded-xl text-[13px]">Reset Perusahaan</button>
            </div>
          </div>
        </div>
      </div>
