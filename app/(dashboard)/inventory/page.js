...
// INVENTORY V15.2 - PAKAI id_halal_19 SAJA (custom_value_2 TIDAK DIPAKAI UNTUK ID HALAL)
export default function InventoryPage(){
  ...
  async function loadMasterAndData(){
    ...
    if(colData) setCustomCols(colData);
  ...
  function handleClickNama(item){
    ...
    setQuickIdHalal(item.id_halal_19 || ""); // PAKAI id_halal_19 SAJA
  ...
  async function handleUpdate(){
    ...
    if(quickPrefix==="Hall" && quickIdHalal && quickIdHalal.length!==19){
      alert("Untuk Hall- wajib ID Halal 19 digit angka!"); return;
    }
    if(quickIdHalal && quickIdHalal.length>0 && !/^\d+$/.test(quickIdHalal)){
      alert("ID Halal 19 digit harus angka saja"); return;
    }

    let kodeBahanBaru = quick.kode_bahan;
    if(quickPrefix !== (quick.kode_prefix||"")){
      const kodeFix = quick._kode_fix;
      const nomor = quick._nomor;
      kodeBahanBaru = quickPrefix ? `${quickPrefix}-${kodeFix}-${nomor}` : `${kodeFix}-${nomor}`;
    }

    // V15.2 FIX: PAKAI id_halal_19 SAJA, custom_value_2 kosongkan / tidak dipakai untuk halal
    const payload = {
      stok: stockBaruTotal,
      stock: stockBaruTotal,
      harga_beli: Number(quickHarga)||0,
      harga_baru: Number(quickHarga)||0,
      custom_value_1: quickMerek || "", // Kolom_1 = Merek
      custom_value_2: "", // TIDAK DIPAKAI UNTUK ID HALAL LAGI
      id_halal_19: quickIdHalal || "", // PAKAI INI SAJA
    ...
    alert(`Update OK pakai id_halal_19 saja:\n${quick.kode_bahan} -> ${kodeBahanBaru}\nMerek: ${quickMerek}\nID Halal 19: ${quickIdHalal||"kosong"}`);
    setQuick(null);
  ...
  function handleTambahPerKategori(kat){ const kodeDefault = masterKode.find(k=>k.kategori_utama===kat)?.kode || "BERAS"; setNewKodeKat(kodeDefault); setNewNama(""); setNewPrefix(""); setNewMerek(""); setNewIdHalal(""); setShowTambah(true); window.scrollTo({top:0, behavior:'smooth'}); }
  ...
  async function handleSimpanTambah(){
    ...
    const payload = {
      ...
      id_halal_19: newIdHalal || "", // PAKAI INI SAJA
      custom_value_1: newMerek || "", // Merek
      custom_value_2: "", // TIDAK DIPAKAI UNTUK HALAL
      satuan: "Kg"
    };
    const { error } = await supabase.from("inventory_items").insert(payload);
    if(error){ alert("Gagal (jalankan SQL id_halal_19 dulu): "+error.message); return; }
    setShowTambah(false); setNewNama(""); await loadMasterAndData();
  }

  const col1Name = "Merek"; // FIX: Kolom_1 tetap Merek

  return (
    <div className="p-4 bg-[#f5f7fb] min-h-screen">
      <div className="bg-slate-900 text-white p-4 rounded-t-xl">
        <div className="text-lg font-bold">Inventory V15.2 - Pakai id_halal_19 SAJA - Merek di custom_value_1</div>
        <div className="text-xs text-slate-300">Live: {items.length} bahan • Kolom: Kode (Hall/Orgk), {col1Name}, id_halal_19 (19 digit) SAJA • Edit klik nama bisa isi Merek + id_halal_19</div>
      ...
      {showTambah && (
        <div className="bg-white border-2 border-green-500 p-4 rounded-xl my-3 shadow">
          <div className="font-bold text-sm mb-2 text-green-700">✅ Tambah Bahan - PAKAI id_halal_19 SAJA</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-yellow-50 border-2 border-yellow-400 p-3 rounded"><div className="text-xs font-bold">Prefix Hall/Orgk</div>
              <select value={newPrefix} onChange={e=>setNewPrefix(e.target.value)} className="w-full p-2 border-2 border-green-500 rounded text-sm font-bold mt-1">
                <option value="">Biasa - BERAS-001</option><option value="Hall">Hall- (wajib id_halal_19 19 digit)</option><option value="Orgk">Orgk- (tanpa halal)</option>
              </select></div>
            <div className="bg-blue-50 border-2 border-blue-400 p-3 rounded"><div className="text-xs font-bold">Kode Kategori</div>
              <select value={newKodeKat} onChange={e=> setNewKodeKat(e.target.value)} className="w-full p-2 border-2 border-blue-500 rounded text-sm font-bold mt-1">
                {masterKode.map(k=>{ const count = items.filter(i=>i._kode_fix===k.kode).length+1; return <option key={k.kode} value={k.kode}>{k.kode} - Next {k.kode}-{String(count).padStart(3,"0")}</option> })}
              </select></div>
            ...
            <div className="bg-purple-50 border-2 border-purple-400 p-2 rounded"><div className="text-xs font-bold">Merek (custom_value_1) - Kolom_1</div><input value={newMerek} onChange={e=>setNewMerek(e.target.value)} placeholder="Sania, Rose Brand" className="w-full p-2 border-2 border-purple-500 rounded text-sm mt-1" /></div>
            <div className="bg-yellow-50 border-2 border-yellow-500 p-2 rounded"><div className="text-xs font-bold">id_halal_19 (19 digit) - PAKAI INI SAJA</div><input value={newIdHalal} onChange={e=>setNewIdHalal(e.target.value)} placeholder={newPrefix==="Hall" ? "Wajib 19 digit" : "Kosong untuk Orgk/biasa"} className="w-full p-2 border-2 border-yellow-500 rounded text-sm mt-1" maxLength={19} /></div>
            ...
            <div className="bg-gray-100 p-2 rounded text-[10px]">Info: custom_value_2 TIDAK dipakai untuk halal lagi - hanya id_halal_19</div>
          </div>
          <div className="mt-3 flex gap-2"><button onClick={handleSimpanTambah} className="flex-1 bg-green-600 text-white py-3 rounded font-bold text-sm">Simpan - {newKodeDisplay} - Merek {newMerek} - id_halal_19 {newIdHalal||"kosong"}</button><button onClick={()=>setShowTambah(false)} className="px-6 py-3 bg-gray-200 rounded text-sm">Batal</button></div>
        </div>
      )}

      {quick && (
        <div className="bg-green-50 border-2 border-green-400 p-4 rounded-xl my-3 shadow-lg">
          <div className="font-bold text-green-800 text-sm">🛠️ Edit Bahan Lama - PAKAI id_halal_19 SAJA: {quick._kode_tampil}</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
            ...
            <div><div className="text-xs font-bold">Prefix Hall/Orgk</div>
              <select value={quickPrefix} onChange={e=>setQuickPrefix(e.target.value)} className="w-full p-2 border-2 border-green-500 rounded text-sm font-bold">
                <option value="">Biasa</option><option value="Hall">Hall- (wajib id_halal_19)</option><option value="Orgk">Orgk- (tanpa halal)</option>
              </select></div>
            ...
            <div className="bg-purple-50 border-2 border-purple-400 p-2 rounded"><div className="text-xs font-bold">Merek (custom_value_1) - EDIT BAHAN LAMA</div><input value={quickMerek} onChange={e=>setQuickMerek(e.target.value)} placeholder="Sania" className="w-full p-2 border-2 border-purple-500 rounded text-sm font-bold mt-1" autoFocus /></div>
            <div className="bg-yellow-50 border-2 border-yellow-500 p-2 rounded"><div className="text-xs font-bold">id_halal_19 SAJA (19 digit) - EDIT BAHAN LAMA</div><input value={quickIdHalal} onChange={e=>setQuickIdHalal(e.target.value)} placeholder={quickPrefix==="Hall" ? "Wajib 19 digit" : "Kosongkan untuk Orgk"} className="w-full p-2 border-2 border-yellow-500 rounded text-sm font-bold mt-1" maxLength={19} /></div>
            <div><div className="text-xs font-bold">Tambah Stock</div><input type="number" value={quickStock} onChange={e=>setQuickStock(e.target.value)} placeholder={`Stock ${quick.stok}`} className="w-full p-2 border-2 border-blue-400 rounded text-sm" /></div>
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={handleUpdate} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded font-bold text-sm">💾 Update - PAKAI id_halal_19 SAJA + Merek</button>
            <button onClick={()=>handleDelete(quick)} className="px-4 py-3 bg-red-600 text-white rounded text-sm font-bold">🗑️ Delete</button>
                    ...
                    <tbody>
                      {list.map(it=>(
                        <tr key={it.kode_bahan} className="border-t hover:bg-blue-50">
                          ...
                          <td className="p-2 text-xs bg-purple-50 font-bold">{it.custom_value_1 || "-"}</td>
                          <td className="p-2 text-xs font-mono bg-yellow-50">{it.id_halal_19 || "-"}</td>
