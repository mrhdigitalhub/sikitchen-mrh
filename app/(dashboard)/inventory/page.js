...
export default function InventoryPage(){
  ...
  function handleClickNama(item){
    setQuick(item); setQuickStock(""); setQuickHarga(String(item.harga_beli||0));
  ...
  async function handleUpdate(){
    ...
    const payload = {
      ...
      harga_beli: Number(quickHarga)||0, harga_baru: Number(quickHarga)||0,
      custom_value_1: quickMerek || "", custom_value_2: "",
      id_halal_19: quickIdHalal || "", kode_prefix: quickPrefix || "",
      kode_lama: quickKodeLama || quick.kode_bahan, kode_bahan: kodeBahanBaru
    };
  ...
  async function handleDelete(item){
    ...
    if(error){ alert("Gagal: "+error.message); return; }
    setItems(items.filter(it=> it.kode_bahan!==item.kode_bahan)); if(quick && quick.kode_bahan===item.kode_bahan) setQuick(null);
  }
  ...
  async function handleSimpanTambah(){
    ...
    const payload = {
      kode_bahan: newKodeDisplay, nama_bahan: newNama.trim(), kategori: newKategoriUtama,
      ...
      id_halal_19: newIdHalal || "", custom_value_1: newMerek || "", custom_value_2: "", satuan: "Kg"
    };
  ...
  return (
    <div className="p-4 bg-[#f5f7fb] min-h-screen">
      <div className="bg-slate-900 text-white p-4 rounded-t-xl">
        <div className="text-lg font-bold">Inventory V15.3 - Pakai id_halal_19 SAJA + Admin Master BALIK - Merek di custom_value_1</div>
        <div className="text-xs text-slate-300">Live: {items.length} bahan • Kolom: Kode, Merek (custom_value_1), id_halal_19 SAJA (19 digit) • Edit klik nama bisa isi Merek + id_halal_19 + Hall/Orgk</div>
      ...
      {showTambah && (
        <div className="bg-white border-2 border-green-500 p-4 rounded-xl my-3 shadow">
          <div className="font-bold text-sm mb-2 text-green-700">✅ Tambah Bahan - id_halal_19 SAJA</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="bg-white border-2 border-green-300 p-2 rounded"><div className="text-xs font-semibold">Nama Bahan *</div><input value={newNama} onChange={e=>setNewNama(e.target.value)} placeholder="Beras Premium" className="w-full p-2 border-2 border-green-400 rounded text-sm mt-1" autoFocus /></div>
      ...
      {quick && (
        <div className="bg-green-50 border-2 border-green-400 p-4 rounded-xl my-3 shadow-lg">
          <div className="font-bold text-green-800 text-sm">🛠️ Edit Bahan Lama - id_halal_19 SAJA + Merek: {quick._kode_tampil}</div>
