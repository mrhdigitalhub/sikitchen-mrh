...
// EXCEL FIXED - 18 KODE SAJA, TIDAK ADA HEWANI, NABATI, HNS, SAUS, LAUK, LAIN, KEMASAN
...
export default function InventoryPage(){
  ...
  // FIX: 1 GARIS LURUS - Kode Bahan = Kode Kategori + No | Kategori Utama AUTO dari Kode (tidak ada kotak kedua)
  ...
  async function loadData(){
    ...
    try{
      ...
      const withNo = data.map((d,i)=> {
        ...
        // FIX SEMUA KODE SALAH: HNS, HEWANI, NABATI, SAUS, LAUK, LAIN, KEMASAN, AYAM, SAYUR, BUMBU, KEJU
        if(raw==="HNS" || raw==="H" || raw==="HEWANI" || raw==="HEWAN" || raw==="NABATI" || raw==="NAB" || ["SAUS","LAUK","LAIN","KEMASAN","AYAM","SAYUR","BUMBU","KEJU"].includes(raw)){
          // Mapping cerdas berdasarkan nama bahan
          if(namaLower.includes("ikan")) kodeFix="IKAN";
          ...
          else if(namaLower.includes("bebek") || namaLower.includes("ayam") || namaLower.includes("sapi") || namaLower.includes("kambing") || namaLower.includes("bebek")) kodeFix="DAGING";
          else {
            // Default mapping untuk HEWANI/NABATI
            if(raw==="HEWANI" || raw==="HEWAN") kodeFix="DAGING";
            ...
            else if(raw==="BUMBU") kodeFix="BUBUK";
            else if(raw==="KEJU") kodeFix="SUSU";
          }
        }

        const utama = MAP_KODE_KE_UTAMA[kodeFix] || "Bahan Baku Utama";
        // Buat kode tampil baru yang benar
        const nomor = d.kode_bahan.split("-")[1] || String(i+1).padStart(3,"0");
        const kodeTampil = `${kodeFix}-${nomor}`;

        return {...d, _no: i+1, _kode_fix: kodeFix, _kode_tampil: kodeTampil, _utama: utama, _raw: raw, selisih: Number(d.harga_baru||d.harga_beli||0) - Number(d.harga_beli||0)}
      });
      setItems(withNo);
  ...
  const filtered = items.filter(i=>{
    ...
    return i._kode_tampil.toLowerCase().includes(s) || i.nama_bahan.toLowerCase().includes(s) || i.kode_bahan.toLowerCase().includes(s);
  });
  ...
  async function handleSimpanTambah(){
    ...
    if(existing){
      ...
      if(error){ alert("Gagal: "+error.message); return; }
      alert(`Berhasil tambah baru ${newKode} - ${newKategoriUtama} - 18 Kode FIXED`);
    }
  ...
  return (
    <div className="p-4 bg-[#f5f7fb] min-h-screen">
      <div className="bg-slate-900 text-white p-4 rounded-t-xl">
        <div className="text-lg font-bold">Master Bahan Sikitchen - V12.8 FIX HEWANI/NABATI + Hapus Kotak Kategori Utama Dobel</div>
        <div className="text-xs text-slate-300">Live: {items.length} bahan • FIX: HEWANI-006 → DAGING-006 / CUMI-001 / UDANG-001 | NABATI-001 → UMBI-001 | HNS → IKAN/UDANG/CUMI/TELUR | Hanya 18 Kode Excel Asli | No Global tetap | Stock 10+5=15</div>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari kode / nama... (18 Kode: BERAS, BOX, DAGING, UMBI, IKAN, UDANG, CUMI, TELUR...)" className="mt-3 w-full p-2 rounded text-black text-sm" />
      </div>

      {showTambah && (
        <div className="bg-white border-2 border-green-500 p-4 rounded-xl my-3 shadow">
          <div className="font-bold text-sm mb-2 text-green-700">✅ FIX V12.8 - Hanya 1 Dropdown 18 Kode (Kotak Kategori Utama Dobel Dihapus)</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="md:col-span-2 bg-yellow-50 border-2 border-yellow-400 p-3 rounded">
              <div className="text-xs font-bold">Kode Kategori Excel (18) - Sudah termasuk Kategori Utama 15 - FIX</div>
              ...
              <div className="text-[11px] text-green-700 mt-2 font-bold">✅ 1 Dropdown = 1 Kode + 1 Kategori Utama (Auto {newKategoriUtama}) | Tidak ada kotak kedua yang bikin salah lagi!</div>
              <div className="text-[10px] text-red-600 mt-1">TIDAK ADA: HEWANI, NABATI, HNS, SAUS, LAUK, LAIN, KEMASAN, AYAM, SAYUR, BUMBU, KEJU</div>
            </div>
            <div className="bg-blue-50 border-2 border-blue-400 p-3 rounded">
              <div className="text-xs font-bold">Kode Bahan Auto FIX</div>
              ...
              <div className="text-[11px] text-blue-600 mt-2">{newKodeKat} = {newKategoriUtama}<br/>No {String(nextNoForKode).padStart(3,"0")}</div>
            </div>
            <div className="bg-white border-2 border-green-300 p-2 rounded">
              <div className="text-xs font-semibold">Nama Bahan * Wajib</div>
              <input value={newNama} onChange={e=>setNewNama(e.target.value)} placeholder="Ex: Tahu Putih, Bebek, Ikan Tuna" className="w-full p-2 border-2 border-green-400 rounded text-sm mt-1" />
            </div>
            <div className="bg-blue-50 border-2 border-blue-400 p-2 rounded">
              <div className="text-xs font-semibold">Stock Awal (10+5=15 logic sama kayak cepat)</div>
              <input type="number" value={newStock} onChange={e=>setNewStock(e.target.value)} className="w-full p-2 border rounded text-sm font-bold mt-1" />
            </div>
            <div className="bg-yellow-50 border-2 border-yellow-400 p-2 rounded">
              <div className="text-xs font-semibold">Harga Beli (Selisih Baru-Lama)</div>
              <input type="number" value={newHarga} onChange={e=>setNewHarga(e.target.value)} className="w-full p-2 border rounded text-sm font-bold bg-yellow-50 mt-1" />
            </div>
          </div>
          <div className="mt-3 flex gap-2"><button onClick={handleSimpanTambah} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded font-bold text-sm">Simpan - {newKode} - {newKategoriUtama} - 1 Garis Lurus FIX</button><button onClick={()=>setShowTambah(false)} className="px-6 py-3 bg-gray-200 rounded text-sm">Batal</button></div>
          <div className="text-[10px] text-gray-500 mt-2">FIX: Kotak Kategori Utama (15) yang dobel sudah dihapus. Sekarang hanya pakai 1 dropdown BERAS - Bahan Baku Utama yang sudah termasuk kategorinya.</div>
        </div>
      )}

      {quick && (
        <div className="bg-green-50 border-2 border-green-200 p-4 rounded-xl my-3">
          <div className="font-bold text-green-800 text-sm">🛠️ Update Cepat: {quick._kode_tampil} - {quick._utama} - No {String(quick._no).padStart(3,"0")} {_raw!==_kode_fix ? ` (RAW ${_raw} → FIX ${_kode_fix})` : ""}</div>
          ...
          return (
            <div key={kat} className="border-b last:border-0">
              <div className="p-3 font-bold text-sm flex justify-between bg-slate-50 cursor-pointer" onClick={()=>setExpanded(prev=>({...prev, [kat]: !isOpen}))}>
                <div>{i+1} &nbsp; {kat} - {list.length} bahan • Contoh: {list.slice(0,4).map(x=>x._kode_fix).join(", ")} {isOpen?"▼":"▶"}</div><div className="text-xs">{isOpen?"Tutup":"Buka"}</div>
              </div>
              {isOpen && (
                <div className="overflow-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-white text-xs"><tr><th className="text-left p-2">No Global</th><th className="text-left p-2">Kode Bahan FIX 18 Kode (Bukan HEWANI/NABATI/HNS)</th><th className="text-left p-2">Nama Bahan</th><th className="text-left p-2">Kategori Utama</th><th className="text-left p-2">Stock</th><th className="text-left p-2">Harga</th><th className="text-left p-2">Aksi</th></tr></thead>
                    <tbody>
                      {list.map(it=>(
                        <tr key={it.kode_bahan} className="border-t hover:bg-blue-50">
                          <td className="p-2 text-xs">{String(it._no).padStart(3,"0")}</td>
                          <td className="p-2 font-mono text-xs font-bold">{it._kode_tampil} {it._raw!==it._kode_fix && <span className="text-[9px] text-red-500">({it._raw}→{it._kode_fix})</span>}</td>
