...
export default function InventoryPage(){
  ...
  async function loadData(){
    ...
    try{
      ...
      const withNo = data.map((d,i)=> {
        ...
        const utama = MAP_KODE_KE_UTAMA[kodeFix] || d.kategori || "Bahan Baku Utama";
        const kodeBaruFix = `${kodeFix}-${String(d.kode_bahan.split("-")[1]||String(i+1).padStart(3,"0"))}`;
        return {...d, _no: i+1, _kode_fix: kodeFix, _kode_tampil: kodeFix===rawKode ? d.kode_bahan : kodeBaruFix, _utama: utama, selisih: Number(d.harga_baru||d.harga_beli||0) - Number(d.harga_beli||0)}
      });
      setItems(withNo);
  ...
  const filtered = items.filter(i=>{
    ...
    return i.kode_bahan.toLowerCase().includes(s) || i.nama_bahan.toLowerCase().includes(s) || i._kode_tampil.toLowerCase().includes(s);
  });
  ...
  async function handleSimpanTambah(){
    ...
    const existing = items.find(i=> i.kode_bahan===newKode || i._kode_tampil===newKode);
  ...
  return (
    <div className="p-4 bg-[#f5f7fb] min-h-screen">
      <div className="bg-slate-900 text-white p-4 rounded-t-xl">
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari kode / nama... (coba IKAN / UDANG / CUMI / TELUR)" className="mt-3 w-full p-2 rounded text-black text-sm" />
      </div>
          ...
          return (
            <div key={kat} className="border-b last:border-0">
              <div className="p-3 font-bold text-sm flex justify-between bg-slate-50 cursor-pointer" onClick={()=>setExpanded(prev=>({...prev, [kat]: !isOpen}))}>
                <div>{i+1} &nbsp; {kat} - {list.length} bahan • Contoh: {list.slice(0,3).map(x=>x._kode_fix).join(", ")} {isOpen?"▼":"▶"}</div><div className="text-xs">{isOpen?"Tutup":"Buka"}</div>
              </div>
              {isOpen && (
